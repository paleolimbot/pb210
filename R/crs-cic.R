...
pb210_crs <- function(cumulative_dry_mass, excess_pb210, excess_pb210_sd = NA_real_,
  ...
  check_mass_and_activity(cumulative_dry_mass, without_errors(excess_pb210), excess_pb210_sd)
  stopifnot(
    inherits(inventory, "pb210_fit_inventory") || is.numeric(inventory),
...
predict.pb210_fit_crs <- function(object, cumulative_dry_mass = NULL, ...) {
  ...
  if (inherits(object$inventory, "pb210_fit_inventory")) {
    inventory_df <- predict(
      object$inventory,
      cumulative_dry_mass = cumulative_dry_mass,
      excess_pb210 = excess_pb210,
      excess_pb210_sd = excess_pb210_sd
    )
    inventory <- inventory_df$inventory
    innventory_sd <- inventory_df$inventory_sd
  } else {
...
#'   [pb210_fit_exponential()] or [pb210_fit_loglinear()] in that the fit's `m` and `b`
...
#' @param object An inventory calculator generated by [pb210_inventory_calculator()].
#'
#' @return [pb210_inventory()] returns a vector with [errors::errors()] of cumulative lead-210 activities
#'   for each sample (in Bq) that can be passed as the `inventory` to [pb210_crs()].
#'   [pb210_inventory_calculator()] returns a fit object that can be used to calculate inventory given an
#'   `cumulative_dry_mass`, `excess_pb210`, and `excess_pb210_sd`. The `predict()` method for the
#'   inventory calculator returns a [tibble::tibble()] with columns `inventory` and `inventory_sd`.
#' @export
#'
#' @examples
#' fake_mass <- 1:10
...
#' @export
pb210_inventory_calculator <- function(model_top = NULL, model_bottom = NULL) {
  model_top <- enquo(model_top)
  model_bottom <- enquo(model_bottom)

  # default model_top
  if (rlang::quo_is_null(model_top)) {
    model_top <- quo(max(.data$cumulative_dry_mass, na.rm = TRUE))
  }

  # default model_bottom
  if (rlang::quo_is_null(model_bottom)) {
    model_bottom <- quo(pb210_fit_exponential(.data$cumulative_dry_mass, .data$excess_pb210))
  }
